...
int GameBooter::BootNintendont(struct discHdr *gameHdr)
{
	...
	u8 ninBBAEmuChoice = game_cfg->NINBBA == INHERIT ? Settings.NINBBA : game_cfg->NINBBA; // ASA
	u8 ninBBAProfileChoice = game_cfg->NINBBAPROF == INHERIT ? Settings.NINBBAPROF : game_cfg->NINBBAPROF; // ASA
	...
	if (NIN_cfg_version >= 9 && ninBBAEmuChoice) //ASA
	{
		nin_config->Config |= NIN_CFG_BBA; // v6.489+ Enable BBA Emulation
		nin_config->BBAProfile = ninBBAProfileChoice; // v6.489+ Which network connection to use for BBAEmu
	}
