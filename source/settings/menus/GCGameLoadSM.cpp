...
void GCGameLoadSM::SetOptionNames()
{
	...
	if(currentGCmode == GC_MODE_NINTENDONT)
	{
		...
		Options->SetName(Idx++, "%s", tr( "BBA Emulation")); // ASA
		Options->SetName(Idx++, "%s", tr( "BBA Network Profile")); // ASA
...
void GCGameLoadSM::SetOptionValues()
{
	...
	if(currentGCmode == GC_MODE_NINTENDONT)
	{
		...
		//! Settings NIN BBA // ASA
		if (GameConfig.NINBBA == INHERIT)
			Options->SetValue(Idx++, tr("Use global"));
		else
			Options->SetValue(Idx++, "%s", tr(OnOffText[GameConfig.NINBBA]));

		//! Settings NIN BBA Profile // ASA
		if (GameConfig.NINBBAPROF == INHERIT)
			Options->SetValue(Idx++, "%s", tr("Use global"));
		else if (GameConfig.NINBBAPROF == 0)
			Options->SetValue(Idx++, "%s", tr("Auto"));
		else
			Options->SetValue(Idx++, "%hi", GameConfig.NINBBAPROF);
...
int GCGameLoadSM::GetMenuInternal()
{
	...
	//! Settings: NIN BAA Profile // ASA
	else if (currentGCmode == GC_MODE_NINTENDONT && ret == ++Idx)
	{
		if (++GameConfig.NINBBAPROF >= 4) GameConfig.NINBBAPROF = INHERIT;
	}
